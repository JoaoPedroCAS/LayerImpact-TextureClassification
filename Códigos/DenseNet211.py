...
# Load DenseNet-201 model and remove the classification layer
class DenseNet201FeatureExtractor(nn.Module):
    def __init__(self):
        super(DenseNet201FeatureExtractor, self).__init__()
        self.model = densenet201(weights=DenseNet201_Weights.IMAGENET1K_V1)
        self.features = nn.Sequential(*list(self.model.children())[:-1])  # Remove classification layer

    def forward(self, x):
        return self.features(x).squeeze()  # Remove unnecessary dimensions
    
    def remove_denseblock(self):
        # Remove the last DenseBlock
        if len(self.features[-1]) > 0:
            denseblocks = list(self.features[-1].children())
            denseblocks.pop()
            self.features[-1] = nn.Sequential(*denseblocks)

    def number_of_denseblocks(self):
        # Return the number of DenseBlocks
        return len(list(self.features[-1].children())) if len(self.features) > 0 else 0

    def remove_last_layer(self):
        # Remove the last layer if exists
...
model = DenseNet201FeatureExtractor().to(device)
num_denseblocks = model.number_of_denseblocks()
camadas_removidas = 0
